...
try, OPTIONS.pred;  catch, OPTIONS.pred  = 1; end % YS added 07/07/21
...
try, beta  = MDP(1).beta;  catch, beta  = 8;    end % policy precision
...
for m = 1:size(MDP,1)
    ...
    for f = 1:Nf(m)
        ...
        % also record fw/bw and bottom=up messages, YS 051320
        QL{m, f} = zeros(Ni,Ns(m,f),1,1,Np(m));
        PX{m, f} = QL{m, f};
        QX{m, f} = QL{m, f};
    end
...
for t = 1:T
    ...
    for m = M(t,:)
        ...
        if isfield(MDP,'link')
                ...
                for g = 1:size(MDP(m).link,2)
                    if ~isempty(MDP(m).link{f,g})
                        ...
                        if OPTIONS.pred
                            ...
                            Nout = length(MDP(m).link{f,g}*O{m}{g,t});
                            mdp.D{f} = ones(Nout, 1)/Nout;
                        end
            ...
            %==============================================================
            MDP(m).mdp(t) = spm_MDP_VB_X(mdp, OPTIONS);
        ...
        if isfield(MDP,'demi')
            ...
            %--------------------------------------------------------------
            MDP(m).dem(t) = spm_MDP_DEM_model(MDP(m).dem(t),...
        ...
        if ~HMM || T == t
                ...
                for i = 1:Ni            % iterate belief updates
                    ...
                    for j = 1:S         % loop over future time points
                        ...
                        for f = 1:Nf(m)
                            ...
                            qL = zeros(Ns(m,f),1);
                            v  = zeros(Ns(m,f),1);
                            px1 = zeros(Ns(m,f),1); px2 = zeros(Ns(m,f),1);
                            qx = zeros(Ns(m,f), 1);
                            ...
                            if dF > exp(-8) || i > 0
                                ...
                                if j < 2
                                    ...
                                    px1 = spm_log(sB{m,f}(:,:,V{m}(j - 1,k,f))*x{m,f}(:,j - 1,k));
                                    v  = v + px1 + qL - qx;
                                end
                                ...
                                %------------------------------------------
                                if j < R
                                    px2 = spm_log(rB{m,f}(:,:,V{m}(j    ,k,f))*x{m,f}(:,j + 1,k));
                                    v  = v + px2 + qL - qx;
                                end
            ...
            if isfield(MDP,'factor')
                ...
                %----------------------------------------------------------
                if sum(H(:)) > - chi && ~isfield(MDP,'VOX') && ~isfield(MDP,'demi')
%                     T = t;
                end
            end
    ...
    for f = 1:Nf(m)
        ...
        for i = 1:T
            for k = 1:Np(m)
                ...
                Ql{m,f}(:,:,1:T,i) = Ql{m,f}(:,:,1:T,i) + QL{m,f}(:,:,1:T,i,k)*u{m}(k,i);
                Px{m,f}(:,:,1:T,i) = Px{m,f}(:,:,1:T,i) + PX{m,f}(:,:,1:T,i,k)*u{m}(k,i);
                Qx{m,f}(:,:,1:T,i) = Qx{m,f}(:,:,1:T,i) + QX{m,f}(:,:,1:T,i,k)*u{m}(k,i);
            end
        end
    end
