function DEM = spm_MDP_DEM_model(DEM,demi,O,o)
% same as spm_MDP_DEM, but use spm_DEM to solve the model, so that the
% outcome (response) can be directly provided and the generative process is
% not involved, and no pU in the resulting DEM structure
...
for t = 1:nt
    ...
    for i = 1:size(po,1)
        for j = 1:size(po,2)
            for k = 1:size(po,3)
                % YS--do not skip the unlikely options
%                 if po(i,j,k) < exp(-3)
%                     F(i,j,k) = - exp(64);
%                 else
                    rE       = demi.U{i,j,k}(:,t);
